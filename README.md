      ...
      <main className="max-w-7xl mx-auto p-4">
        ...
        {view === 'orders' && (
          <div className="bg-white p-6 rounded shadow-sm">
            <h2 className="font-bold text-xl mb-6">सभी ऑर्डर्स</h2>
            <div className="space-y-4">
              {orders.map(o => (
                <div key={o.id} className="border p-4 rounded-lg flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                  <div>
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded uppercase ${o.status === 'Pending' ? 'bg-yellow-100 text-yellow-700' : 'bg-green-100 text-green-700'}`}>{o.status}</span>
                    <h4 className="font-bold mt-1">{o.productName}</h4>
                    <p className="text-xs text-gray-500">कस्टमर ID: {o.userId?.substring(0,8)} | ₹{o.price}</p>
                    <p className="text-xs mt-1 italic text-gray-400">पता: {o.address}</p>
                  </div>
                  <div className="flex gap-2">
                    {o.status === 'Pending' && (
                      <button onClick={async () => await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'orders', o.id), {status: 'Delivered'})} className="bg-green-600 text-white px-4 py-1 rounded text-xs font-bold">डिलीवर किया</button>
                    )}
                    <button onClick={() => {setSelectedChatUser(o.userId); setView('chat_seller')}} className="border border-blue-600 text-blue-600 px-4 py-1 rounded text-xs font-bold">चैट करें</button>
                  </div>
                </div>
              ))}
            </div>
